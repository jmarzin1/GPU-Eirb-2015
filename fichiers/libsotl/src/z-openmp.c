...
void omp_init (sotl_device_t *dev)
{
  ...
  exit (1);
#endif
  boites = malloc(sizeof(unsigned int)*27);
  borders_enabled = 1;
...
static void get_box_coord(const sotl_domain_t *dom,  const calc_t x, const calc_t y, const calc_t z,const calc_t rrc, const calc_t &box_x, const calc_t &box_y, const calc_t &box_z){
