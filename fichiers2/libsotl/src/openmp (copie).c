...
static void omp_force (sotl_device_t *dev)
{
  ...
  sotl_atom_set_t *set = &dev->atom_set;
  //atom_set_sort(set);
  //printf("%lf",LJ_RCUT_DEFAULT_VALUE);
  #pragma omp parallel for
  for (int current = 0; current < set->natoms; current++) {
    ...
    other = current;
    while((other<set->natoms)/*&&(abs(set->pos.z[current]-set->pos.z[other]) <= LJ_RCUT_DEFAULT_VALUE)*/){
      //printf("ici %u %u\n",current, other);
      if (current != other) {
	//printf("ici2 %u %u\n",current, other);
    ...
    other = current;
    while((other>0)/*&&(abs(set->pos.z[current]-set->pos.z[other]) <= LJ_RCUT_DEFAULT_VALUE)*/){
      //printf("la\n");
      if (current != other) {
	calc_t sq_dist = squared_distance (set, current, other);
