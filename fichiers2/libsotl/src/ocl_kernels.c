...
void scan_down_step(sotl_device_t *dev){
  ...
  for (int i = 1 ; i < nb_group ; i++){
    ...
    err |= clSetKernelArg (dev->kernel[k], 1, sizeof (cl_mem), &i);
    check(err, "Failed to set kernel arguments: %s", kernel_name(k));
...
{ 
  ...
  if(begin > end) // Silly code to avoid warning 
    ...
    calc_t * local_box_buf = malloc ((dev->domain.total_boxes+1)* sizeof(calc_t));
    
    err |= clSetKernelArg (dev->kernel[k], 0, sizeof (cl_mem), dev->box_buffer);
    err |= clSetKernelArg (dev->kernel[k], 1, sizeof (cl_mem), local_box_buf);
    check(err, "Failed to set kernel arguments: %s", kernel_name(k));
